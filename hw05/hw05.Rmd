...
```
I will try to filter out the rows with Oceania as their continent from the gapminder dataset and drop the respective level. The number of rows, the number of levels, and the levels before and after are shown bellow:
...
Now I will reorder the continent variable by the variance of life expectancy. Then I will arrange the datasets and show the effect of reordering on arranging the datasets by the continent variable. It can be observed that after reordering, arrange function arranges the dataset based on the new order. Furthermore, the plots number of entries for each continent before and after of the reordering are shown.
